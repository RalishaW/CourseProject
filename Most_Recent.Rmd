---
title: "Data-driven identification of transcription factor networks \\linebreak over time
...
```{r, echo= FALSE, fig.width=17.5, fig.height=16, fig.align='right'}
...
geom_point(alpha = 0.4) + geom_smooth(method="lm", fill=NA, size = 1.2) + theme_bw() + labs(title="Relationship Between AXL and Fra2")+ theme(plot.title = element_text(hjust = 0.5, face="bold"))

summary(lm(MiTFg ~ ATF4+phenotype, data=df_0.5_d0_outcome))$coefficients
...
:::: {.column width="70%"} 
\small
```{r, echo= FALSE, out.width="240px", out.height="150px", message = FALSE}
LR_AXL_ex
```
...
```{r, echo= FALSE, fig.width=16, fig.height=10, message = FALSE}
...
* Lot of # Transistory States found on Drug Vem + Tram 
...
* Confounding variables - multicollinearity of transcription factors, bias when chosing the predictor.

#### Classification Trees

* Overtting into that may lead to do wrong prediction, bias from a subjective observer.

#### Two hypothesis Testing

* Type I and type II error(unlikely), not providing enough information or causal relation.
